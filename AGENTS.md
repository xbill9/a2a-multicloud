...
This workspace contains several agents implemented using ADK. They are designed to demonstrate the A2A (Agent-to-Agent) protocol and multi-agent orchestration.

### Primary Agents (under `src/agents/`)
- **A2A Hello World**:
  - **Location**: `src/agents/a2a_hello_world/`
  - **Role**: Simple agent with tools for weather and time.
  - **Tools**: `get_weather`, `get_current_time`.
- **A2A Events**:
  - **Location**: `src/agents/a2a_events/`
  - **Role**: Uses Google Search to find local events.
- **A2A Master Agent**:
  - **Location**: `src/agents/a2a_master_agent/`
  - **Role**: A master agent that delegates tasks to other agents in the same group.

### Poly-Orchestration Agents (under `poly-python/agents/`)
- **Poly Master Agent**:
  - **Location**: `poly-python/agents/poly_master/`
  - **Role**: Orchestrates sub-agents written in Python, Go, and JS.
  - **Sub-agents**: `primecheck_agent` (Go), `primegenerator_agent` (JS), `rand_agent` (Python).
- **Poly Rand Agent**:
  - **Location**: `poly-python/agents/poly_rand/`
  - **Role**: Generates random numbers, even numbers, and odd numbers.
  - **Tools**: `get_random_even_number`, `get_random_odd_number`, `get_random_number`.

### Infrastructure Agents
- **A2A Agent Card**:
  - **Location**: `a2a-agentcard/`
  - **Role**: Serves the `agent_card.json` required for A2A discovery.
