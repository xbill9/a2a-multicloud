# Gemini Workspace for `a2a-hello-world`

You are a Python Agent Developer working with Google Cloud.
You should follow Python Best practices.
The recommended version is 3.13.

You will use the ADK and the A2A protocol.

the agent init file  __init__.py
should only contain from . import agent

the adk python repo is here: https://github.com/google/adk-python
the adk documentation is here: https://google.github.io/adk-docs/

# Agent Development Kit (ADK)

This project is a multi-agent system demonstration using ADK and A2A.

## Project Structure

The project is organized into several agent groups:

- `src/agents/`: Primary Python-based agents.
  - `a2a_hello_world`: Basic weather and time agent.
  - `a2a_events`: Event search agent using Google Search Tool.
  - `a2a_master_agent`: Orchestrates other agents in `src/agents/`.
  - `a2a_weather_time`: Focused weather and time retrieval.
  - `a2aeventsnyc`: Specialized events agent for NYC.
- `poly-python/agents/`: Agents participating in a cross-language "Poly" orchestration.
  - `poly_master`: Python-based master agent that orchestrates Python, Go, and JS agents.
  - `poly_rand`: Python-based random number generation agent.
- `a2a-agentcard/`: Handles Agent Card distribution for A2A discovery.
- `a2a-client-test/`: Contains a test client for verifying A2A communication.

## Key Development Commands

- `source set_env.sh`: Initialize environment variables.
- `./web.sh`: Launch ADK Web UI for the main agents.
- `./poly-master.sh`: Run the Poly Master orchestration.
- `./inspect.sh`: Launch the A2A Inspector (requires sibling `a2a-inspector` repo).
