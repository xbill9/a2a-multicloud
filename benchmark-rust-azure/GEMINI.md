# Gemini Workspace for `benchmark-rust-azure`

Welcome! This guide is designed for AI coding assistants working in the `benchmark-rust-azure` sub-project of the Multi-Cloud A2A (Agent-to-Agent) Prime Calculation Benchmark system.

---

## 1. Role and Context
You are a Rust developer working within a multi-cloud agent structure. This directory hosts the **Azure Container Apps (ACA)** agent. 
...
### Entrypoint and Routes: [src/main.rs](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/src/main.rs)
- **HTTP Server**: Uses Axum and Tokio listener.
- **Routes**:
  - `GET /health`: Basic ping returning `OK`.
  - `GET /.well-known/agent.json` and `GET /.well-known/agent-card.json`: Returns capability information (skills list) using the [AgentCard](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/src/main.rs#L38-L51) struct.
...
- **State management**:
  ...
  - [CalculationGuard](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/src/main.rs#L14-L27) uses RAII to automatically toggle the `CALCULATION_ACTIVE` state off when the calculation finishes or panics.

### Dependency Configuration: [Cargo.toml](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/Cargo.toml)
- Set to **Rust 2021 edition** (as configured in the Cargo manifest).
- Crucial dependencies:
  - `num-bigint`: Arbitrary-precision integer arithmetic for $2^p - 1$ calculations used in [is_mersenne_prime](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/src/main.rs#L126-L145).
  - `axum`: Modern, lightweight web framework built on top of Hyper and Tokio.
  - `tokio`: Async engine, configured with `full` features.

### Containerization: [Dockerfile](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/Dockerfile)
...
- Deploys automatically via Azure CLI toolchain to Azure Container Apps, using a unique container app name based on the system hostname.
...
### Keeping State Safe
- Because Container Apps are serverless and can scale down/up or spin up multiple instances, the atomic state tracker `CALCULATION_ACTIVE` is localized to each instance.
...
All commands can be invoked from the [Makefile](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/Makefile).
...
| `make status` | Query health and deployment status of local server and ACA container |
| `make test` | Run Rust unit tests |
| `make test-a2a` | Run A2A integration test script ([test-a2a.sh](file:///home/xbill/a2a-multicloud/benchmark-rust-azure/test-a2a.sh)) |
| `make docker-build` | Build local Docker image |
| `make deploy` | Build, push to Azure Container Registry (ACR), and deploy to Azure Container Apps (ACA) |
| `make a2a` | Test deployed endpoint with a JSON-RPC status check payload |
| `make logs` | Tail Azure Container App logs |
| `make destroy` | Clean up ACR registry, Container App, Environment, and Azure Resource Group |
